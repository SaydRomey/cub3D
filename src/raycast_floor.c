...
/*   Updated: 2024/03/06 15:44:30 by cdumais          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_ceiling_floor(t_cub *cub, int y)
{
	...
	while (x < WIDTH)
	{
		...
		pos.x += step.x;
		pos.y += step.y;
		color = get_pixel(cub->texture[F], tex_pos[0], tex_pos[1]);
		draw_pixel(cub->img, x, y, color);
		color = get_pixel(cub->texture[C], tex_pos[0], tex_pos[1]);
		draw_pixel(cub->img, x, HEIGHT - y - 1, color);
		x++;
	}
}
